...
class Code(object):
    ...
    def _clone(self):
        ...
        """
        return envoy.run("git clone %s %s" % (self.build.repo_url, self.build.cwd))

    def _reset(self):
        """
        git reset --hard refspec
        """
        return envoy.run("cd %s && git reset --hard %s" % (self.build.cwd, self.build.refspec))

    def update(self):
        if self._which_git():
            if path.exists(self.build.cwd):
